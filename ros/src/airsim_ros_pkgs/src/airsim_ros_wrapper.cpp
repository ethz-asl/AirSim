...
void AirsimROSWrapper::initialize_airsim() {
    ...
    try {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            airsim_client_ = std::move(
                    std::unique_ptr<msr::airlib::RpcLibClientBase>(new msr::airlib::CarRpcLibClient(host_ip_)));
        }
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json() {
    ...
    for (const auto &curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        auto isLidar = std::function < bool(
        const SensorPublisher &pub)>([](const SensorPublisher &pub) {
    ...
    if (vehicle_name_ptr_map_.size() > 1 && airsim_mode_ == AIRSIM_MODE::DRONE) {
        ...
        vel_cmd_group_world_frame_sub_ = nh_private_.subscribe("group_of_obots/vel_cmd_world_frame", 1,
...
void AirsimROSWrapper::update_and_publish_static_transforms(VehicleROS *vehicle_ros) {
    if (vehicle_ros && !vehicle_ros->static_tf_msg_vec.empty()) {
        ...
        vehicle_ros->static_tf_msg_vec.clear();
    }
}
